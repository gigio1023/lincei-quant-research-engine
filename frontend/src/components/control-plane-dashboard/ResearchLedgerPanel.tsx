      ...
      <div className="flex flex-col gap-3 border-b border-[#2b3139] p-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        >
          {model.runningBaselineResearch
            ? t("Running dry-run backtest")
            : t("Run dry-run backtest")}
        </button>
      </div>
      ...
      <div className="p-4">
        ...
        {model.visibleResearchRuns.length === 0 ? (
          <div className="rounded-lg border border-[#2b3139] bg-[#0b0e11] p-4 text-sm font-semibold text-[#929aa5]">
            {t("No research runs recorded yet.")}
          </div>
        ) : (
            ...
            <table className="w-full min-w-[760px] border-collapse text-left text-sm">
              <thead className="text-[11px] uppercase text-[#707a8a]">
                <tr className="border-b border-[#2b3139]">
                  <th className="py-2 pr-4">{t("Run")}</th>
                  <th className="py-2 pr-4">{t("Backtest Metrics")}</th>
                  <th className="py-2 pr-4">{t("Benchmark")}</th>
                  <th className="py-2 pr-4">{t("Drawdown")}</th>
                  <th className="py-2 pr-4">{t("Evidence")}</th>
                </tr>
              </thead>
                  ...
                  <tr key={run.id}>
                    ...
                    <td className="py-3 pr-4 align-top">
                      ...
                      <div className="text-xs text-[#707a8a]">
                        ...
                        {t("Trades")} {run.backtestMetrics.tradeCount}
                      </div>
                    </td>
                        ...
                        <div className="mt-2 space-y-1">
                          {run.datasetRefs.map((datasetRef) => (
                            ...
                            >
                              ...
                              <div>
                                {datasetRef.provider ?? t("unknown provider")} /{" "}
                                {datasetRef.source ?? t("unlabeled source")}
                              </div>
                              <div>
                                {datasetRef.windowStart} {t("to")}{" "}
                                {datasetRef.windowEnd}
                              </div>
                              <div>
                                {t("available")}{" "}
