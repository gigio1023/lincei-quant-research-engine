...
- the dashboard shows Funding Readiness, Broker Write Readiness, Broker Snapshot Monitor, and Broker Fill Evidence next to paper execution so the user can see whether expected deposit, pilot-budget gates, external account truth, and paper state agree. The dashboard defaults to English and includes a one-click Korean toggle for operator review;
