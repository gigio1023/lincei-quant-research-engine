...
- `live_pilot_readiness_records` stores the next gate after funding readiness: tiny pilot budget, per-order cap, funding record link, broker adapter readiness, schema migration policy, credential custody, read-only/fill polling, and broker emergency cancel/flatten/open-order-poll blockers. This is evidence only and keeps `brokerWriteEnabled`, `orderEndpointImplemented`, `brokerExecutionEnabled`, and `liveTradingEnabled` false;
- `POST /control-plane/funding-readiness/:id/assess-live-pilot-readiness` and `GET /control-plane/live-pilot-readiness` expose that live pilot readiness ledger. The current implementation intentionally remains `blocked` because live order placement, broker write access, production signed order-plan custody, and broker emergency controls are not implemented;
- `broker_order_commands` stores the first provider-neutral broker command ledger. It can prepare a blocked dry-run command from a signed paper order plan, or record dry-run emergency cancel/flatten intent, but it never calls a broker endpoint and hard-sets `brokerExecutionEnabled/liveTradingEnabled: false`;
- `POST /control-plane/paper-order-plans/:id/prepare-broker-order-command`, `POST /control-plane/broker-order-commands/emergency-dry-run`, and `GET /control-plane/broker-order-commands` expose that command ledger. Requests reject credentials, raw account refs, live payloads, and caller-supplied orders; all order intents are copied from the stored paper plan and blocked by live-pilot/readiness evidence;
...
- the dashboard shows Funding Readiness, Broker Write Readiness, Broker Order Command Ledger, Broker Snapshot Monitor, and Broker Fill Evidence next to paper execution so the user can see whether expected deposit, pilot-budget gates, proposed broker commands, external account truth, and paper state agree. The dashboard defaults to English and includes a one-click Korean toggle for operator review;
