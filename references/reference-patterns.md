...
| Deterministic baseline runner | `references/projects/backtrader/backtrader/analyzers/returns.py`, `references/projects/backtrader/backtrader/analyzers/drawdown.py`, `references/projects/backtrader/backtrader/analyzers/tradeanalyzer.py`, `references/projects/vectorbt/vectorbt/portfolio/base.py` | Bootstrap with a pure deterministic backtest runner that computes returns, benchmark comparison, drawdown, turnover, and trade metrics before introducing external data or trained models. |
...
| Execution order/fill ledger | `references/projects/nautilus_trader/nautilus_trader/model/events/order.pyx`, `references/projects/alpaca-py/alpaca/trading/models.py`, `references/projects/freqtrade/freqtrade/persistence/trade_model.py` | Paper plans should preserve client ids, order-line ids, fill ids, fill timestamps, quantities, prices, fees, status, and cash impact instead of only summary booleans. |
| Accounting from fills | `references/projects/nautilus_trader/nautilus_trader/accounting/manager.pyx`, `references/projects/hummingbot/hummingbot/connector/budget_checker.py` | Cash and position ledgers should be computed from fills and pre-trade reservations before any broker write path exists. |
| Durable paper account state | `references/projects/nautilus_trader/nautilus_trader/accounting/manager.pyx`, `references/projects/alpaca-py/alpaca/trading/models.py`, `references/projects/freqtrade/freqtrade/persistence/trade_model.py` | Keep a local paper account snapshot across cycles, but evolve JSON ledgers into append-only account, position, reservation, and fill-derived accounting tables before live trading. |
| Reconciliation loop | `references/projects/nautilus_trader/nautilus_trader/live/execution_engine.py` | Even local paper execution should compare expected vs actual cash/positions with tolerances, then later swap actuals to broker read-only snapshots. |
