...
- Define the core gap before implementation. For this repository, the gap should usually map to the alpha/execution validation loop, not to a cosmetic or speculative surface.
...
- Prefer vertical slices that prove the validated capital-allocation loop end to end: data -> alpha -> LEAN insight -> portfolio target -> risk -> paper trading/shadow trading order intent -> fill report or would-have-traded record -> reconciliation. Horizontal infrastructure is allowed only when it unblocks that slice.
...
- Explain implications when they matter. For example, distinguish "simulator passed" from "strategy validation artifacts passed", "pre-trade risk check blocked by policy" from "command failed", and "legacy identifier kept for compatibility" from "term approved for new work".
...
- Always identify the core functional gap before implementation. In this project, the current core gap is QuantConnect Cloud promotion evidence plus point-in-time LLM-derived feature replay, not the absence of more UI panels.
...
- When adding docs, keep them implementation-directive: define schemas, commands, acceptance criteria, and blockers. Avoid vague strategy prose that does not move the engine forward.

## Parallelization Discipline

- Always consider whether research, data ingestion, feature generation, LLM-derived feature jobs, ablations, backtest sweeps, or QuantConnect Cloud artifact imports can run as bounded parallel jobs.
- Parallelize only before promotion and execution. Portfolio target consolidation, risk cuts, paper trading/shadow trading execution intents, reconciliation, pre-trade risk checks, and future broker writes must remain single-writer and fail closed.
...
- Do not let parallel sweeps become multiple-testing bias. Store failed, blocked, flat, and losing variants alongside winners.
...
- Keep the LLM inside the alpha loop through typed LLM-derived features and risk judgments. Do not let it cross into broker credentials, raw order payloads, or final order quantities.
...
- Prefer clear names, small functions, and schemas over inline narration.

### What to comment (required in this repo)

| Situation                                                                                                                        | What to write                                                                                                                        |
| -------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------ |
| **Broker-write or execution-like paths** (broker, paper trading/shadow trading execution, reconciliation, pre-trade risk checks) | The safety invariant, fail-closed default, and the failure mode being prevented (e.g. duplicate submit, stale features, cap bypass). |
| **Boundary crossings** (LEAN ↔ NestJS, LLM ↔ broker, script ↔ CLI)                                                            | Why the boundary exists and what must _not_ happen across it (no OpenAI in LEAN backtest, no credentials in LLM/frontend/logs).      |
| **Policy gates** (pre-trade risk check, kill switch, schema verification flags)                                                  | Why the gate exists and what "unknown" means (always blocked).                                                                       |
...
- **Docs** (`docs/`, `SPEC.md`, module READMEs): Schemas, API contracts, runbooks, acceptance criteria, and operator commands.
...
- State maximum notional caps and "blocked beats ready" wherever paper trading, shadow trading, pre-trade risk check, or broker paths are implemented.
...
- Dashboards must be operational surfaces, not marketing pages. Show state, blockers, ledger artifacts, and next safe actions in one scan-friendly layout.
...
- This repository is a non-production research engine. The primary proof is direct execution of the relevant engine path: LEAN backtest, QuantConnect Cloud backtest when available, import, alpha replay, paper cycle, shadow trading cycle, pre-trade risk check, or reconciliation. Unit tests support that proof; they do not replace it.
- Prefer Detroit/classicist, state-based unit tests where they are high value: pure scoring logic, portfolio/risk policy, schema validation, timestamp/lookahead checks, idempotency, cap enforcement, and fail-closed broker/pre-trade risk check behavior.
...
- If a change touches broker, execution, paper account, risk, reconciliation, shadow trading, or pre-trade risk check paths, include at least one blocked/failure case. Unknown state must stay blocked.
