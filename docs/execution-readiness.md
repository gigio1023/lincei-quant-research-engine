...
- broker-grade transaction isolation, quantity, cost-basis, reservation, and realized-PnL accounting.
...
| Paper execution         | Started | Deterministic paper simulator ledger, durable signed paper approvals, explicit paper account seed/promote controls, append-only account events, idempotent paper-execute endpoint, durable local paper account state, local reconciliation, and execution-control state exist. Production signing custody, transaction isolation, quantity/cost-basis accounting, and broker-backed reconciliation are still missing. |
| Broker read-only        | Partial | Manual read-only broker snapshot ledger, paper reconciliation, and a provider-neutral Toss adapter readiness contract exist. Verified Toss/client polling and credential isolation are still missing.                                                                                                                                                                                                                  |
...
The repo now has a provider-neutral read-only broker snapshot ledger and broker adapter readiness contract. It can store imported cash/holdings evidence, reject credentials and order payloads, reconcile the snapshot against paper account state, and show whether Toss credentials/schema/sandbox/read-only gates are satisfied. It still does not have a verified Toss API client, credential boundary, polling schedule, or any order endpoint.
