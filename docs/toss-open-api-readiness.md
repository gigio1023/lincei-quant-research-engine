...
The raw OpenAPI schema URLs are referenced by the official developer docs app, but direct shell requests from this environment returned `403` responses. That means the exact endpoint schema, enum values, rate limits, sandbox behavior, and error models are not verified in this repo yet.
